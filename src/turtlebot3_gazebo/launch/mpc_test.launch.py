...
pkg_turtlebot3_controller = get_package_share_directory('turtlebot3_controller')
...
def generate_launch_description():
    ...
    include_gazebo = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
          ...
          launch_arguments = {
              'world': PathJoinSubstitution([pkg_turtlebot3_gazebo,'worlds', world_name]),
              'verbose': verbose,
              'gui': 'false',
          }.items()
    )
    ...
    xacro_file_path = os.path.join(pkg_turtlebot3_gazebo, 'urdf', 'turtlebot3_waffle.urdf.xacro')
    ...
    if use_rviz:
        rviz = Node(
            package='rviz2',
            executable='rviz2',
            arguments=['-d', [os.path.join(pkg_turtlebot3_gazebo, 'rviz', 'config.rviz')]]
        )
        ld.add_action(rviz)
